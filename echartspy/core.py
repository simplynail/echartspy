...
class Chart(object):
    ...
    def generate_tag_div_chart(self):
        return _str_htmldiv_chart_boilerplate.format(uid=self.uid,
                                                     div_style=str(self._div_style)[1:-1].replace("'", "").replace(",",
                                                                                                                   ";"))
        ...
        if self._async_series != []:
            if not self._async_url:
                raise AttributeError(
                    'async data defined but no url provided for async query. Use "set_asunc_url" method to define it')
            async_series_str = ''
            for name in self._async_series:
                async_series_str = async_series_str + _str_js_async_pointer.format(name=name)
            async_query = _str_js_chart_async_data_boilerplate.format(url=self._async_url, uid=self.uid,
                                                                      series_pointers=async_series_str)
            js_chart = js_chart + '\n' + async_query
